...
const ReadBadge = ({ isRead }: { isRead?: boolean }) => (
  <span
  ...
  >
    {isRead ? <MailOpen className="w-3 h-3" /> : <Mail className="w-3 h-3" />}
    {isRead ? "Read" : "New"}
  </span>
);
...
const Messages = () => {
  ...
  const items = messagesResponse?.data ?? [];
  const meta = messagesResponse?.meta ?? { total: 0 };

  const { data: summaryResponse, isLoading: summaryLoading } =
    useMessageSummary();
  const summary = summaryResponse?.data ?? { total: 0, unread: 0, read: 0 };
    ...
    () => [
      {
        ...
        width: "60px",
        center: true,
        cell: (_row: Message, rowIndex: number) => (
      ...
      },
      {
        ...
        cell: (row) => (
          <div>
            <p
              className={`text-xs font-light ${!row.isRead ? "text-black font-medium" : "text-black/80"}`}
            >
      ...
      {
        name: "Service",
        selector: (row) => row.serviceOfInterest,
        sortable: true,
        cell: (row) => (
          <span className="text-xs text-black/70 font-light">
            {row.serviceOfInterest || "—"}
          </span>
        ),
        width: "160px",
      },
      {
        ...
        cell: (row) => (
          <p className="text-xs text-black/50 font-light line-clamp-2 max-w-xs">
            {row.message}
          </p>
        ),
        grow: 2,
      ...
      {
        name: "Status",
        selector: (row) => (row.isRead ? "read" : "new"),
        sortable: true,
        cell: (row) => <ReadBadge isRead={row.isRead} />,
        width: "90px",
      },
      {
        name: "Date",
      ...
      },
      {
        ...
        cell: (row) => (
          <RowActionMenu
            actions={[
              { icon: Eye, label: "Read Message", onClick: () => setSelected(row) },
            ]}
          />
        ),
      },
  ...
  const handleExport = () => {
    ...
    doc.setFontSize(16);
    doc.text("Messages Report", 14, 15);
    doc.setFontSize(10);
    doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, 22);
    autoTable(doc, {
      startY: 28,
      head: [["Name", "Email", "Phone", "Service", "Message", "Date"]],
      body: items.map((m) => [
    ...
    doc.save(`messages_export_${Date.now()}.pdf`);
  };
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                <ReadBadge isRead={selected.isRead} />
                ...
                <section>
                  ...
                  <div className="space-y-3">
                    ...
                    <DrawerRow label="Phone" value={selected.phoneNumber} />
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="space-y-3">
                    <DrawerRow
                      label="Service"
                      value={selected.serviceOfInterest || "—"}
                    />
                  </div>
                </section>
